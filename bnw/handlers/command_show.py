...
def set_subscriptions_info(request, messages):
    ...
    subscriptions = objs.Subscription.find({
...
def showSearch(parameters, page, request):
    ...
    # BRAIN-DAMAGED IDIOT
    messages = [x.filter_fields() for x in (objs.Message.find_sort(
        parameters, [('date', pymongo.DESCENDING)], limit=20, skip=page * 20))]
    messages = set_subscriptions_info(request, messages)
    messages.reverse()
    return dict(ok=True, format="messages", cache=5, cache_public=True,
                messages=messages)


def showComment(commentid):
        comment = objs.Comment.find_one({'id': commentid})
        if comment is None:
            return dict(ok=False, desc='No such comment',
                        cache=5, cache_public=True)
        return dict(ok=True, format='comment', cache=5, cache_public=True,
                    comment=comment.filter_fields())


def showComments(msgid, request, bl=None, after=''):
    message = objs.Message.find_one({'id': msgid})
    if message is None:
        return dict(
            ok=False, desc='No such message', cache=5, cache_public=True)
    if request.user:
        user = request.user['name']
        subscribed = objs.Subscription.count({
    ...
    if bl:
        qdict['user'] = {'$nin': bl}
    if after:
        after_comment = objs.Comment.find_one({'id':msgid+'/'+after.split('/')[-1]})
        if after_comment:
            qdict['date'] = {'$gte': after_comment['date']}
    comments = objs.Comment.find_sort(
    ...
    if replies:
        if not message:
            return dict(
                ...
                cache=3600)
        return showComments(message, request, bl, after)
    else:
        ...
        if before:
            befmsg = objs.Message.find_one({'id': before})
            if befmsg:
                parameters['date'] = {'$lt': befmsg['date']}
            else:
                return dict(ok=False, desc="Message to search before doesn't exist.")

        if after:
            afmsg = objs.Message.find_one({'id': after})
            if afmsg:
                parameters['date'] = {'$gt': afmsg['date']}
            else:
                return dict(ok=False, desc="Message to search after doesn't exist.")
        return showSearch(parameters, int(page), request)
...
def cmd_feed(request, page="0"):
    ...
    feed = objs.FeedElement.find_sort({'user': request.user['name']},
                                            [('_id', pymongo.DESCENDING)], limit=20, skip=page * 20)
    messages = [x.filter_fields() for x in (objs.Message.find_sort({'id': {'$in':
                                                                                 [f['message']
                                                                                     for f in feed]
                                                                                 }}, [('date', pymongo.ASCENDING)]))]
    return dict(ok=True, format="messages",
             messages=messages,
             desc='Your feed',
             cache=5)
...
def cmd_today(request, use_bl=False):
    ...
    bl = get_user_bl(request, use_bl)
    last_rebuild = objs.GlobalState.find_one({'name': 'today_rebuild'})
    if not last_rebuild:
        last_rebuild = {'name': 'today_rebuild', 'value': 0}
    rebuild = time.time() > TODAY_REBUILD_PERIOD + last_rebuild['value']
    if rebuild:
        objs.GlobalState.mupdate({'name': 'today_rebuild'}, {'name': 'today_rebuild', 'value': time.time()}, True)

        start = time.time() - 86400
        objs.Today.remove({})
        result = objs.Comment.map_reduce(TODAY_MAP, TODAY_REDUCE, out='today', query={'date': {'$gte': start}})
        print 'map_reduce result', result
    if (not rebuild) or result:
        for x in range(10):
            postids = [x['_id'] for x in (objs.Today.find_sort({}, [('value', -1)], limit=20))]
            if len(postids)>0: break
        qdict = {'id': {'$in': postids}}
        if bl: qdict['user'] = {'$nin': bl}
        dbposts = dict(
            (x['id'], x.filter_fields())
            for x in (objs.Message.find(qdict)))
...
def cmd_today2(request):
    """ Показать обсуждаемое за последние 24 часа """
    start = time.time() - 86400
    messages = [x.filter_fields() for x in (objs.Message.find_sort({'date': {'$gte': start}}, [('replycount', pymongo.DESCENDING)], limit=20))]
    messages.reverse()
    return dict(ok=True, format="messages",
             messages=messages,
             desc='Today''s most discussed',
