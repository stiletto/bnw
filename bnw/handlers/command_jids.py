...
def cmd_jids(request, add="", delete="", select=""):
    ...
    if add:
        ...
        if other_user:
            defer.returnValue(
                dict(ok=False, desc='JID is being used by other user.'))
        if len(request.user['jids']) > 16:
            defer.returnValue(
                dict(ok=False, desc='Why would you need SO MANY JIDs?'))
        if len(request.user['pending_jids']) > 5:
            defer.returnValue(
                dict(ok=False, desc='Too many pending JIDs aargh.'))

        request.user['pending_jids'].append(add)
        _ = yield request.user.save()
        defer.returnValue(dict(ok=True, desc='Please send "confirm %s" from JID being added to confirm JID ownership.' %
                          (request.user['name'], )))
    if delete:
        if delete in request.user['jids']:
            if delete == request.user['jid']:
                defer.returnValue(
                    dict(ok=False, desc='Cannot delete active jid.'))
...
def cmd_confirm(request, code):
    ...
    if not teh_user:
        defer.returnValue(
            dict(ok=False, desc='This JID wasn''t added for this user.'))
