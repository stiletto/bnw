...
#        (ur'L @(?P<user>\S+)','not_implemented'),
        ...
        (ur'(?P<anonymous>(ANON|ФТЩТ)\s+)?[#№](?P<message>[0-9A-Za-z]+)\s(?P<text>.+)',command_post.cmd_comment),
        ...
        (ur'(?:((?P<anonymous>ANON|ФТЩТ)\s+)?(?:\s*(?P<tag1>[\*!]\S+))?(?:\s+(?P<tag2>[\*!]\S+))?(?:\s+(?P<tag3>[\*!]\S+))?(?:\s+(?P<tag4>[\*!]\S+))?(?:\s+(?P<tag5>[\*!]\S+))?\s+)?(?P<text>.+)',
